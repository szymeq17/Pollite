...
class PollMapperTest {
    ...
    public void shouldCorrectlyMapPollToDto() {
        ...
        Assertions.assertThat(result.getId()).isEqualTo(POLL_ID);
        Assertions.assertThat(result.getText()).isEqualTo(POLL_TEXT);
        Assertions.assertThat(result.getStartDateTime()).isEqualTo(POLL_START_DATE);
        Assertions.assertThat(result.getEndDateTime()).isEqualTo(POLL_END_DATE);
        Assertions.assertThat(result.getOwnerUsername()).isEqualTo(USERNAME);
        Assertions.assertThat(result.getPollAnswers()).containsAll(List.of(pollAnswerDto1, pollAnswerDto2, pollAnswerDto3));
    }
